   ...
   $.ajax({
       ...
       success: function(data){ //js파일에 연결이 완료되면 자동으로 호출되는 함수
           ...
           $('#btn').click(function(e){
                ...
                $('.product_btn a').removeClass('current');
           ...
           //각각의 메뉴 버튼 클릭 시
           $('.btn1').click(function(e){  //커피류
            e.preventDefault();
 
            newArray = useData.filter(function(element){
              //console.log($('#title').val());
              //return element.Name == $('#title').val();
              return element.type=='커피류';
             });
            dataPrint(newArray);
            $('.product_btn a').removeClass('current');
         ...
         $('.btn2').click(function(e){  //간식류
            e.preventDefault();
 
            newArray = useData.filter(function(element){
              //console.log($('#title').val());
              //return element.Name == $('#title').val();
              return element.type=='간식류';
             });
            dataPrint(newArray);
            $('.product_btn a').removeClass('current');
             $(this).addClass('current');
         ...
         $('.btn3').click(function(e){  //기타음료
            e.preventDefault();
 
            newArray = useData.filter(function(element){
              //console.log($('#title').val());
              //return element.Name == $('#title').val();
              return element.type=='기타음료';
             });
            dataPrint(newArray);
            $('.product_btn a').removeClass('current');
             $(this).addClass('current');
