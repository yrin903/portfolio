...
$(document).ready(function() {
   ...
   $(".menu_ham, #headerArea .box").click(function(e) { //메뉴버튼 클릭시
    ...
    //       $('#headerArea').css('background','rgba(255,255,255)');  //top보여라~~~~
    //       $('#headerArea').addClass('down');
    //   }else{
    //       $('#headerArea').css('background','rgba(255,255,255,0.4)');//top안보여라~~~~
...
//페밀리 사이트 이동 코드
$('.family_site .arrow').toggle(function(e){//홀수번째 -> 리스트가 보인다.
   e.preventDefault();
   $('.family_site  .aList').fadeIn('slow');
   $(this).children('span').html('<i class="fa-solid fa-chevron-down"></i>');

},function(e){ //짝수번째 -> 리스트가 안보인다.
   e.preventDefault();
   $('.family_site .aList').fadeOut('fast');
